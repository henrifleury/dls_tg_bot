...
async def task_wrapper(semaphore: Semaphore, f_path, *args):
    ...
    try:
        ...
        # TODO check file size and raise exception if > 10Mb (tg limit 10Mb - will not be send)

        data = {'chat_id': chat_id, 'caption': 'enhanced photo'}
        url = f'{API_URL}{BOT_TOKEN}/sendPhoto?chat_id={chat_id}'
        with open(f_path, "rb") as img_f:
            res = requests.post(url, data=data, files={"photo": img_f})
        # TODO check res.json() and raise exception if something wrong
        logger.info(f'{f_name} send to chat: {chat_id}, as reply to message: {message_id}')
        await remove(f_path)
...
async def main(img_dir=RESULT_FOLDER, max_n_threads=MAX_N_THREADS, wrapper=task_wrapper, delay=DELAY):
    ...
    while True:
        ...
        tasks = []
        logger.info(f'start processing list: {img_list[:threads_nbr]}, {len(img_list)} images in queue')
